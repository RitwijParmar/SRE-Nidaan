...
"$GCLOUD_BIN" run deploy sre-nidaan-brain \
  ...
  --set-env-vars "MODEL_ID=${MODEL_ID},HF_PRODUCTION_REPO_ID=${HF_PRODUCTION_REPO_ID},PRODUCTION_ARTIFACT_LABEL=${PRODUCTION_ARTIFACT_LABEL},MAX_MODEL_LEN=1024,GPU_MEMORY_UTILIZATION=0.82,VLLM_ENFORCE_EAGER=1,SERVING_BACKEND=transformers,TRANSFORMERS_LOAD_IN_4BIT=1,HF_HUB_ETAG_TIMEOUT=120,HF_HUB_DOWNLOAD_TIMEOUT=120" \
...
"$GCLOUD_BIN" run deploy sre-nidaan-body \
  ...
  --max-instances 5 \
  --timeout 300 \
  --set-env-vars "VLLM_ENDPOINT=${BRAIN_URL}/v1,MODEL_ID=${MODEL_ID},PRODUCTION_ARTIFACT_LABEL=${PRODUCTION_ARTIFACT_LABEL},GENERATION_CANDIDATES=1,GENERATION_MAX_TOKENS=512,VLLM_REQUEST_TIMEOUT_SECONDS=90,VLLM_MAX_RETRIES=0"
