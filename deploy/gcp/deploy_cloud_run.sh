...
"$GCLOUD_BIN" run deploy sre-nidaan-body \
  ...
  --max-instances 5 \
  --timeout 300 \
  --set-env-vars "VLLM_ENDPOINT=${BRAIN_URL}/v1,MODEL_ID=${MODEL_ID},PRODUCTION_ARTIFACT_LABEL=${PRODUCTION_ARTIFACT_LABEL},GENERATION_CANDIDATES=1,GENERATION_MAX_TOKENS=384,VLLM_REQUEST_TIMEOUT_SECONDS=90,VLLM_MAX_RETRIES=0"
