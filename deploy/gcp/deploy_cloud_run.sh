...
"$GCLOUD_BIN" run deploy sre-nidaan-brain \
  ...
  --set-env-vars "MODEL_ID=${MODEL_ID},HF_PRODUCTION_REPO_ID=${HF_PRODUCTION_REPO_ID},PRODUCTION_ARTIFACT_LABEL=${PRODUCTION_ARTIFACT_LABEL},MAX_MODEL_LEN=1024,GPU_MEMORY_UTILIZATION=0.82,VLLM_ENFORCE_EAGER=1,SERVING_BACKEND=transformers,TRANSFORMERS_LOAD_IN_4BIT=1" \
